...
class Gate:
    def __init__(self, gate_setting_tree):
        ...
        for gate_map in self.GateMapping:
            ...
            setattr(
                self.GateMapping[gate_map]["EntranceSslContext"], "DomainName", gate_map
            )
        print(self.gateSettingTree)
        print(self.DestinationsWeight)

    async def onEntranceToDestination(
        self, B, entrance_connection, destination_connection
    ):
        return B

    async def onDestinationToEntrance(
        self, B, destination_connection, entrance_connection
    ):
        return B
    ...
    async def __getDestinationName__(self, entrance_connection):
        return entrance_connection._Writer.get_extra_info(
            "ssl_object"
        ).context.DomainName

    async def __opengate__(self, entrance_connection, destination_connection):
        await asyncio.gather(
            self.__entranceHandler__(entrance_connection, destination_connection),
            self.__transportHandler__(destination_connection, entrance_connection),
        )
    ...
    async def __entranceHandler__(self, entrance_connection, destination_connection):
        try:
            while entrance_connection.isOnline():
                ...
                event_handling_result_buf = await self.onEntranceToDestination(
    ...
    async def __transportHandler__(self, destination_connection, entrance_connection):
        try:
            while destination_connection.isOnline():
                ...
                event_handling_result_buf = await self.onDestinationToEntrance(
    ...
    async def __proxyHandler__(self, entrance_connection):
        ...
        await self.__opengate__(entrance_connection, destination_connection)
    ...
    async def __start__(self):
        ...
        server = await asyncio.start_server(
            self.__proxyInitHandler__,
            self.EntranceHost,
            self.EntrancePort,
            ssl=entrance_ssl_context,
        )
        async with server:
...
class EFI:
    class Bravegate:
        ...
        async def __entranceHandler__(
            self, entrance_connection, destination_connection
        ):
            try:
                while entrance_connection.isOnline():
                    buf = await entrance_connection.Recv()
        ...
        async def __transportHandler__(
            self, destination_connection, entrance_connection
        ):
            try:
                while destination_connection.isOnline():
                    buf = await destination_connection.Recv()
        ...
        async def __openDestination__(self):
            reader, writer = await asyncio.open_connection(
                self.gateMapping["Host"], self.gateMapping["Port"]
            )
        ...
        async def __start__(self):
            server = await asyncio.start_server(
                self.__proxyInitHandler__, self.EntranceHost, self.EntrancePort
            )
            async with server:
                await server.serve_forever()
    ...
    class Stablegate:
        ...
        async def __openDestination__(self, destination_name):
            ...
            destination = self.gateMapping[destination_name]["Destinations"][
                minimum_index
            ]
            destination_context = ssl.create_default_context()
            reader, writer = await asyncio.open_connection(
    ...
    class Gate:
        def __init__(self, gate_setting_tree):
            ...
            for gate_map in self.gateMapping:
                ...
                self.EntranceSslContextTree[gate_map] = self.gateMapping[gate_map][
                    "EntranceSslContext"
                ]
        ...
        async def __gateInitHandler__(self, reader, writer):
            ...
            if server_name is None:
                server_name = [x for x in self.EntranceSslContextTree][0]
            ...
            if entrance_connection.isTryingHandshake:
                if destination["SSL"] is not None:
                    ...
                    if len(entrance_connection.ALPN) > 0:
                        destination_context.set_alpn_protocols(entrance_connection.ALPN)
                    reader, writer = await asyncio.open_connection(
                        destination["Host"],
                        destination["Port"],
                        ssl=destination_context,
        ...
        async def __start__(self):
            server = await asyncio.start_server(
                self.__gateInitHandler__, self.EntranceHost, self.EntrancePort
            )
            async with server:
                await server.serve_forever()
