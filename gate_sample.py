...
class Nova(n0va.Service):
    def __init__(self, host, port, name: bytes) -> None:
        ...
        self.connections = {}
        self.GetFunctions = {"/GetTest.get": self.GetTest}
        self.Name = name

    async def GetTest(self, connection, Request, ReplyHeader):
        ReplyHeader["ReplyContent"] = b":".join([b"GET", self.Name, Request["content"]])
...
clover = Nova(
    host=Config.Clover.Host, port=Config.Clover.Port, name=b"clover.lobeliasecurity.com"
)
ivy = Nova(host=Config.Ivy.Host, port=Config.Ivy.Port, name=b"ivy.lobeliasecurity.com")
azami = Nova(
    host=Config.Azami.Host, port=Config.Azami.Port, name=b"azami.lobeliasecurity.com"
)


async def start():
    await asyncio.gather(
        clover.__Start__(), ivy.__Start__(), azami.__Start__(), gate.__Start__()
    )
